...
public class Athlete {
	...
	public Athlete(Long athleteId,
			@NotNull(message = "* SSN is required") @Pattern(regexp = "\\d{3}-\\d{2}-\\d{4}", message = "* SSN must be a valid SSN formated as 123-45-6789.") String ssn,
			@NotBlank(message = "* Full name is a required field") String fullName,
			@Pattern(regexp = "((\\(\\d{3}\\) ?)|(\\d{3}-))?\\d{3}-\\d{4}", message = "* Contact number must be a valid phone number formated as (123) 456-7890.") String phoneNumber,
			@NotEmpty(message = "* Date of birth is required.") LocalDate dateOfBirth,
			@NotEmpty(message = "* Date of registration is required.") LocalDate dateOfRegistration,
			@NotEmpty(message = "* Number of medals won is required.") int totalNumberOfMedalsWon,
			@NotNull(message = "* Monthly salary is required") @Digits(integer = 9, fraction = 2, message = "* Monthly salary must be a numeric/monetary amount in decimal (money) format as 'xxx.xx'") Double monthlySalary,
